...
//         console.log(gamepadAPI.axesStatus[1]*gamepadAPI.axesStatus[0]) //debugging
    ...
    class Eovlight {
        ...
        reproduce(){
            let eov = new Eovlight(this.body.x, this.body.y, this.r+((Math.random()-.5)*5),this.g+((Math.random()-.5)*5),this.b+((Math.random()-.5)*5),this.body.radius+((Math.random()-.5)*.5))
            eov.attack = this.attack+((Math.random()-.5)*.1)
            eov.health = this.maxhealth+((Math.random()-.5)*10)
        ...
        draw(){
            for(let t = 0;t<lights.length;t++){
                if(this!=lights[t]){
                    if(this.children.includes(lights[t]) || lights[t] == this.parent){
                        continue
                    }
    ...
    let lights = []
    for(let t = 0;t<100;t++){
        lights.push(new Eovlight(Math.random()*canvas.width, Math.random()*canvas.height, Math.random()*255,Math.random()*255,Math.random()*255, (Math.random()*8)+1))
    }
